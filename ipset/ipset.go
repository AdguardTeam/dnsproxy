...
func New(name string, o *Options) (*IPSet, error) {
	...
	if !strings.HasPrefix(o.HashType, "hash:") {
		return nil, fmt.Errorf("not a hash type: %s", o.HashType)
	}
...
// Add the specified entry to the set from DNS answer.
func (s *IPSet) Parse(answer dns.RR) bool {
	var err error
	...
	} else if answerType == dns.TypeAAAA {
		if rr, ok := answer.(*dns.AAAA); ok {
			if rr.AAAA != nil {
				ip = &rr.AAAA
			}
		}
	...
	if ip != nil {
		addr := ip.String()
		if err = s.Add(addr, s.Timeout); err != nil {
			log.Error("Failed ot push %s to ip set %s. %s", addr, s.Name, err.Error())
			return false
		} else {
			log.Debug("Pushed %s to ip set %s", addr, s.Name)
			return true
		}
	}
