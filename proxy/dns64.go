...
// least a single AAAA answer not within the DNS64 prefixes or a CNAME/DNAME.
//
...
func (p *Proxy) filterNAT64Answers(rrs []dns.RR) (filtered []dns.RR, hasAnswers bool) {
	...
	for _, ans := range rrs {
		...
		case *dns.AAAA:
			...
			} else if netutil.SliceSubnetSet(p.dns64Prefs).Contains(addr) {
				// Filter the record.
				continue
			} else {
...
// shouldStripDNS64 returns true if DNS64 is enabled and req is a PTR request
// for an address having either one of custom DNS64 prefixes or the Well-Known
// one.
//
...
func (p *Proxy) shouldStripDNS64(req *dns.Msg) (ok bool) {
	...
	switch {
	case netutil.SliceSubnetSet(p.dns64Prefs).Contains(addr):
